"""family_sgolf3_6 -- cheaper EXACT solvers for LOCAL, origin-anchored rules via a
...
params only. That is far cheaper than the multi-op baselines these tasks carry.
...
model is emitted. Nothing depends on locally-seen grid sizes -- the same Conv is
value-exact for any grid the generator can produce.
"""
...
def _fit_single_conv(tins, out_grids, K):
    ...
    if uxy.shape[0] != ux.shape[0]:
        return None  # same neighbourhood -> two colours: not local at radius R
    if uxy.shape[0] > 4000:
