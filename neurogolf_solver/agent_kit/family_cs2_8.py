"""family_cs2_8 — FINAL COMPLETE-SWEEP minimal recompile.

One out_blend6 incumbent is *correct in logic* but scores 0 in the ORT-1.23.2
grader because the InferenceSession refuses to load it:

  * task218 (90c28cc7): ir_version 13, which exceeds ORT 1.23.2's max
    supported IR version (11) — the model is rejected before any op runs.
    It *also* carries a uint8 Min(13) node ("masked_colors"); ORT 1.23.2 has
    no uint8 kernel for Min, so even with a fixed IR version the session would
    fail to build.

Both are fixed by a numerically-identical recompile (identical to the patch
used in family_cs2_4):

  * ir_version is clamped to 10 (the value the sibling loadable incumbents
    use) so the model passes ORT's IR-version check.
  * uint8 Min  ->  Cast operands to fp16, Min in fp16, Cast back to uint8.
    All label/coordinate values here are small non-negative integers (colors
    0..9 and 0) that are exact in fp16, so the result is bit-for-bit identical.

Everything else — every op, initializer, and shape — is untouched, so the
patched graph produces exactly the incumbent's intended output.  It is exact
on train+test+arc-gen and on fresh generator samples, and scores strictly
