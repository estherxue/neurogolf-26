...
def main():
    ...
    src = os.path.join(KIT, sys.argv[2] if len(sys.argv) > 2 else "out_blend13")
    dst = os.path.join(KIT, sys.argv[3] if len(sys.argv) > 3 else "out_blend14")
    ...
    print(f"\n{os.path.basename(dst)} total={rep['total_points']} (delta +{total_delta:.2f})  zip={zp}")
